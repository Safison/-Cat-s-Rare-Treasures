...
def get_treasures(sort_by = 'age', order='ASC', colour = None, max_age=None, min_age=None):
    conn = None
    try:
        where_queries = []
        conn = connect_to_db()
        where_colour = ''
        if colour:
            where_queries.append(f"WHERE colour={literal(colour)}")
        if max_age:
            where_queries.append(f"WHERE age <= {literal(max_age)}")
        if min_age:
            where_queries.append(f"WHERE age >= {literal(min_age)}")
            
        query_string = f"SELECT * FROM treasures {' and '.join(where_queries)} ORDER BY {identifier(sort_by)} {identifier(order)}"
...
def update_price(treasure_id,treasure:Treasure):
    ...
    #print("updated treasure", updated_treasure)
    treasure_col = [col['name'] for col in conn.columns]
...
def delete_treasure(treasure_id:int):
    ...
    deleted_treasure = conn.run(query_string, treasure_id = treasure_id)
